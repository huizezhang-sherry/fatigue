...
ll <- function(par, data){
  ...
  ss_pred = par[7] + par[8]*con 
  u = f$speed - ss_pred
  f <- -sum(log(dnorm(u, par[9], par[10])))
  return(f)
}

ll(c(-0.1,-1,-2,3,3.2,5,180,10,0,20), f)
fit <- optim(c(-0.1,-1,-2,3,3,3,180,20,0,10), ll, data = f)
...
conc_plot <- conc_plot %>% mutate(con = conc_1 + conc_brk1_sum + conc_brk2_sum) %>% 
  mutate(ss_pred = fit$par[7] + fit$par[8]*con)


ggplot(conc_plot, aes(y= ss_pred, x = f.speed)) + 
  geom_point() + 
  scale_x_continuous(limits = c(min(conc_plot$f.speed), max(conc_plot$f.speed))) + 
  scale_y_continuous(limits = c(min(conc_plot$f.speed), max(conc_plot$f.speed))) + 
  theme(aspect.ratio=1) 

ggplot(conc_plot, aes(x = f.t_n)) + 
  geom_point(aes(y = ss_pred, col = "red")) + 
...
ggplot(conc_plot) + geom_line(aes(x = f.t_n, y = ss_pred)) + 
  ...
  scale_y_continuous(limit = c(160, 170))
...
ll <- function(par, data){
  ...
  ss_pred = par[7] + par[8]*con + par[9]*f$impt_high + par[10]*f$impt_low + par[11]*f$rally
  u = f$speed - ss_pred
