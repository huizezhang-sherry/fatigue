...
ll <- function(par, data){
  ...
  u = f$speed - ss_pred
  f <- -sum(log(dnorm(u, par[11], par[12])))
  return(f)
}

ll(c(-6, -4,-3, 3, 3.2,5,180,20,10,10,0,10), f)
fit <- optim(c(-0.1, -1, -1, 3,3.2, 5,180,20,10,10,0,10), ll, data = f)
...
# plot
conc_0<- data.frame(f$t_n, conc_1, conc_2, conc_3)
ggplot(conc_0) + geom_line(aes(x = f$t_n, y = conc_2))
conc_plot <- cbind(conc_0, conc_brk1_sum, conc_brk2_sum)
...
ggplot(conc_plot) + geom_line(aes(x = f.t_n, y = ss_pred)) + 
  geom_vline(data = filter(conc_plot, brk1==1), aes(xintercept = f.t_n, alpha = 0.5), col = "red", linetype = "dashed") +
  geom_vline(data = filter(conc_plot, brk2 ==1), aes(xintercept = f.t_n, alpha = 0.5), col = "blue")
