...
RF <- RF %>% mutate(Count = seq(1,length(RF$SetNo),1)) %>% filter(RF$ServeNumber != 0)
  
# Extract DoubleFault data


DoubleFault<- RF_all %>% filter(RF_all$ServeNumber == 0) %>% mutate(Round = )
...
ggplot(RF,aes(x=Count,y=Speed_KMH,col = SetNo)) +
  ylim(min(RF$Speed_KMH),max(RF$Speed_KMH))+
  geom_point(aes(group = SetNo)) +
...
```{r}
# model serving speed via dose-response model 

### drc fails because of convergence reason 
library(drc)
fit_0 <-  drm(RF$Speed_KMH~RF$Count, fct = LL.4()) 
mselect(fit, list(LL.3(), LL.5(), W2.4()))


library(dr4pl)
dr <- dr4pl(Speed_KMH~Count, data = RF)
plot(dr)
```
