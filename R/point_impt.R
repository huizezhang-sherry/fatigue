...
point_impt <- function(dt,name){
  ...
  break1 <- break_point %>% 
    # if player 2 is supplied, win< 0 mean PointServer = 1GameWinner = 2, thus the supplied player breaks the game
    filter(player2 == name) %>% filter(win<0)%>% 
    # chain the match_num and SetNo to form single index
    mutate(index = as.numeric(paste(match_num,SetNo, sep = "")))
  
  break2 <- break_point %>% 
    # if player 1 is supplied, win> 0 mean PointServer = 2,GameWinner = 1, thus supplied breaks the game
    filter(player1 == name) %>% filter(win>0)%>% 
    # chain the match_num and SetNo to form single index
    mutate(index = as.numeric(paste(match_num,SetNo, sep = "")))
  ...
  # select the columns from origin dataset for left_join
  f <- dt %>% select(match_num, SetNo, point,index)
  ...
  point_impt = ifelse(dt$point %in% point_impt$point_point,1,0)
  return(point_impt)
}

point_impt = point_impt(Federer, 'Roger Federer')
